...
public class DriveToTargetCommand extends Command {
  ...
  public DriveToTargetCommand(
      Drivetrain drivetrain, VisionSubsystem visionSubsystem, int targetId, double targetDistance) {
  ...
  public void execute() {
    ...
    if (visionSubsystem.CameraConnected() && visionSubsystem.getTargetVisible(targetId)) {
      ...
      double forwardsbackwardsOutput =
          forwardsbackwardsPidController.calculate(
              visionSubsystem.getTargetSpacePose(targetId).getZ());
      double leftrightOutput =
          leftrightPidController.calculate(-visionSubsystem.getTargetSpacePose(targetId).getX());
      System.out.println(
          rotationPID.atSetpoint() + ", " + forwardsbackwardsPidController.atSetpoint());
